...
- 转发到统一 IMAP 收件箱的邮箱池：在“邮箱池 → 导入”选择“转发到同一 IMAP 收件箱”，每行仅填原邮箱地址；填写统一收件箱地址（163 邮箱通常使用 `imap.163.com`、SSL `993`），再在“配置 → 邮箱 / OTP”保存该收件箱的 IMAP 客户端授权码。已经注册 GPT 的邮箱可勾选“导入后标记为已注册 GPT 账号”，已有邮箱池素材也会标记已用并关联账号。此类素材存入通用 IMAP 池，注册新账号时启用 `USE_EMAIL_SERVICE=True` 并将 `EMAIL_SOURCE` 包含 `imap`。收码只接受邮件头或转发正文中明确指向该原邮箱地址、且在本次请求之后到达的验证码邮件。
